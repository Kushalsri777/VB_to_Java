...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[proc_claim_insert]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[proc_claim_insert]
GO
...
/* Date: 10/14/2002
  * Author: Betsy Walker
  *
  * Purpose: This stored procedure inserts a row into the CLAIM_T table.
  *
  * Logic flow:
  *             Verify supplied @clm_num does not already exist in the CLAIM_T table
  ...
  *             Perform the insert of the row into the CLAIM_T table
  *
  ...
  *   03/06/03 BAW - Added support for new CLM_FOR_RES_DTH_IND column and
  *	                 nullability of iss_st_cd and insd_dth_res_st_cd
  */
CREATE PROCEDURE proc_claim_insert(@admn_syst_cd dom_admin_cd
...
BEGIN
    ...
    BEGIN
        SELECT @Error_Message = 'Claim Number: ' + ISNULL(@clm_num, '<NULL>') + ' already exists in the CLAIM_T table.'
    ...
    INSERT INTO claim_t(admn_syst_cd
            ,clm_insd_dth_dt
            ,clm_insd_first_nm
            ,clm_insd_last_nm
            ,clm_insd_ssn_num
    ...
    ELSE
    BEGIN
        SELECT @Error_Message = 'An error happened while inserting to the CLAIM_T for the Claim Number: ' + ISNULL(@clm_num, '<NULL>')
        RAISERROR(@Error_Message, 16, 1)
        RETURN 4028
    END
...
GRANT EXECUTE ON [dbo].proc_claim_insert TO AppRoleClaims
GO
GRANT EXECUTE ON [dbo].proc_claim_insert TO Support
GO
