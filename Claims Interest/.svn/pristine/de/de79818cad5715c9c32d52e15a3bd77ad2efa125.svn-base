...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[proc_claim_lu_select3]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[proc_claim_lu_select3]
GO
...
**                      order to populate the SSN Lookup combo box on the Insured
...
CREATE PROCEDURE dbo.proc_claim_lu_select3 WITH RECOMPILE AS
...
BEGIN
    ...
    SELECT clm_insd_ssn_num
        ,clm_num
        ,clm_id
    FROM claim_t                            
    WHERE clm_insd_ssn_num IS NOT NULL
    ORDER BY clm_insd_ssn_num, clm_num
...
GRANT EXECUTE ON dbo.proc_claim_lu_select3 TO AppRoleClaims
GO
GRANT EXECUTE ON dbo.proc_claim_lu_select3 TO Support
GO
